...
#now = datetime.now()
now1 = datetime.now()
current_date = now1.strftime("%x")
#current_time = now.strftime("%H:%M:%S") #now.strftime("%X") #datetime.now().time() #time.ctime()
ts = now1.strftime("%m%d%Y")

# Store the filename in a global variable
#filename = ts + '-Stranger' + '-TPRecord.xlsx'
filename = 'Stranger-TPRecord.xlsx'
...
def submit_data():
    ...
    try:
        ...
        # Date everytime fill up data
        sheet.cell(row=sheet.max_row, column=3, value=current_date)

        # Time everytime fill up data
        now = datetime.now()
        current_time = now.strftime("%H:%M:%S")
        sheet.cell(row=sheet.max_row, column=4, value=current_time)
